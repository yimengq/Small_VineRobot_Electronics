...
REG_FAULT      = 0x00
REG_CONFIG0    = 0x09
REG_CONFIG3    = 0x0C  # IMODE[1:0] (bits 7:6), INT_VREF (bit 4)
REG_CONFIG4    = 0x0D

REG_REG_CTRL0  = 0x0E  # REG_CTRL[1:0] at bits 4:3
...
class MCP4725:
    ...
    Uses MCP4725 "Fast Mode" 2-byte write:
      [ 00 PD1 PD0 D11 D10 D9 D8 ] [ D7 ... D0 ]
    """
    ...
    def set_voltage(self, vout: float) -> float:
        ...
        return (code / 4095.0) * self.vdd  # actual (quantized)

    def set_current_limit(self, amps: float, ripropi_ohm: float, aipropi_uA_per_A: float) -> tuple[float, float]:
        """
        ITRIP * AIPROPI = VVREF / RIPROPI  => VVREF = ITRIP * AIPROPI * RIPROPI
        """
        amps = float(amps)
...
class DRV8234:
    def __init__(self, bus: SMBus, addr: int, name: str = "M"):
        self.bus = bus
        self.addr = addr
        self.name = name
        self.last_dir = None
...
def print_help():
    ...
    print("  f [counts]           -> forward on active motor")
    print("  r [counts]           -> reverse on active motor")
    print("  brake|stop           -> brake active motor")
    print("  status               -> status active motor")
    print("  1f/1r/1brake/1status  -> command motor 1 explicitly")
    print("  2f/2r/2brake/2status  -> command motor 2 explicitly")
    print("  brakeall             -> brake both motors")
    print("  statusall            -> status both motors")
    print("  ilimit <amps>        -> set current limit (shared DAC => both drivers)")
    print("  vref <volts>         -> set VREF directly (shared DAC)")
    print("  dac <0..4095>        -> write raw DAC code")
    print("  clim                 -> show current-limit estimate (from DAC code)")
...
def main():
    ...
    ap.add_argument("--addr1", type=lambda x: int(x, 0), default=0x30, help="Motor1 DRV8234 7-bit I2C addr (default: 0x30)")
    ap.add_argument("--addr2", type=lambda x: int(x, 0), default=0x32, help="Motor2 DRV8234 7-bit I2C addr (default: 0x32)")
    ...
    print("\n=== Dual DRV8234 Move-by-Counts + Shared DAC Current Limit ===")
    print(f"Using /dev/i2c-{args.bus}")
    print(f"Motor1 addr=0x{args.addr1:02X}, Motor2 addr=0x{args.addr2:02X}, DAC addr=0x{args.dac_addr:02X}")

    with SMBus(args.bus) as bus:
        dev1 = DRV8234(bus, args.addr1, name="M1")
        dev2 = DRV8234(bus, args.addr2, name="M2")

        print("Probing motor drivers...")
        ok1 = dev1.probe()
        ok2 = dev2.probe()
        if not ok1:
            print("No ACK from Motor1 DRV8234.")
        if not ok2:
            print("No ACK from Motor2 DRV8234.")
        if not (ok1 and ok2):
            sys.exit(1)
        ...
        have_dac = dac.probe()
        if have_dac:
            print("DAC ACK OK (shared VREF)")
        else:
            print("WARNING: DAC not found; current-limit commands disabled (VREF may be floating!).")

        reg_ctrl = 0b01 if args.cbc else 0b00

        print("Configuring Motor1...")
        setup_driver(dev1, reg_ctrl=reg_ctrl, disable_ovp=True)
        print("Configuring Motor2...")
        ...
        if have_dac:
            ...
            print(f"Startup current limit (shared): ITRIP≈{itrip_act:.3f} A (VREF={vref_act:.3f} V, DAC={dac.last_code}/4095)")

        print("\nReadbacks:")
        for d in (dev1, dev2):
            print(f"{d.name}: CONFIG0=0x{d.read_reg(REG_CONFIG0):02X} CONFIG3=0x{d.read_reg(REG_CONFIG3):02X} "
                  f"CONFIG4=0x{d.read_reg(REG_CONFIG4):02X} REG_CTRL0=0x{d.read_reg(REG_REG_CTRL0):02X}")
            d.print_status()

        active = 1
        print_help()
        print("Ready. Examples: `m 2` then `f 2000`, or `1f 500`, `2status`, `brakeall`.")
        ...
        try:
            while True:
                ...
                # allow prefixes like "1f", "2status"
                explicit_dev = None
                if cmd0 and cmd0[0] in ("1", "2") and len(cmd0) >= 2:
                    explicit_dev = int(cmd0[0])
                    cmd0 = cmd0[1:]  # rest of command
                dev = get_dev(explicit_dev if explicit_dev else active)
                ...
                if cmd0 == "statusall":
                    dev1.print_status()
                    print(f"[M1] RC_CNT={dev1.read_count16()}  SPEED={dev1.read_speed()}")
                    dev2.print_status()
                    print(f"[M2] RC_CNT={dev2.read_count16()}  SPEED={dev2.read_speed()}")
                    continue
                ...
                if cmd0 == "clim":
                    ...
                    print(f"Shared current limit estimate: ITRIP≈{itrip:.3f} A (VREF={vref:.3f} V, DAC={dac.last_code}/4095)")
                    continue
                ...
                if cmd0 in ("ilimit", "ilim"):
                    ...
                    print(f"Shared ITRIP≈{itrip_act:.3f} A (VREF={vref_act:.3f} V, DAC={dac.last_code}/4095)")
                    continue
                ...
                if cmd0 == "vref":
                    ...
                    print(f"Shared VREF={vref_act:.3f} V -> ITRIP≈{itrip_act:.3f} A (DAC={dac.last_code}/4095)")
                    continue
                ...
                if cmd0 == "dac":
                    ...
                    vref = (dac.last_code / 4095.0) * dac.vdd
                    itrip = (vref / args.ripropi) / (args.aipropi * 1e-6)
                    print(f"Shared DAC={dac.last_code}/4095 => VREF={vref:.3f} V -> ITRIP≈{itrip:.3f} A")
                    continue

                # Move commands (per selected/explicit motor)
                if cmd0 in ("f", "r"):
