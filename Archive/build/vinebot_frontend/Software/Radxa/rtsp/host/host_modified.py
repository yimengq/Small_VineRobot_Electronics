...
Base-station host that:
- Reads a ROS /joy topic (run joy_node separately)
- Streams RTSP video from the Radxa
- Sends pressure setpoints over UART
...
#
# The /cmd endpoint is convenient during bring-up/debug since it also supports
# any other CLI commands the Radxa server accepts.
...
def joy_cb(msg: Joy):
    with lock:
        ...
        state["tmotor_vel"] = v_min + v_max * (raw + 1.0)
        ...
        if 0 <= BTN_BRAKE_ALL < len(msg.buttons) and _edge(state["prev_buttons"][BTN_BRAKE_ALL], msg.buttons[BTN_BRAKE_ALL]):
            ...
            # also clear pending so we don't immediately command motion again
            state["m1_pending"] = 0
            state["m2_pending"] = 0
            print("[motor] BRAKE")
...
def motor_integrator_loop():
    ...
    while not rospy.is_shutdown():
        ...
        # Match old servo mapping: pushing stick up (a1 positive) resulted in decreasing angle.
        # Keep that directionality unless you want to flip it.
...
def _send_motor_counts(m1: int, m2: int):
    if USE_TEXT_CMD_ENDPOINT:
        # Send as two separate text commands.
        if m1 != 0:
...
def health_poll_loop():
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            # Render a short summary without assuming a strict schema.
            if isinstance(js, dict):
                # common-ish keys people use
